...
function loadfile(f) {
	...
	function getJSONData() {
			...
			for (var i= 1; i < data.length; i++) {
					if (data[i].Vente < 8) {
						data[i].Tranche = "0 - 8"
					} else if (data[i].Vente < 16) {
						data[i].Tranche = "08 - 16"
					} else if (data[i].Vente < 24) {
						data[i].Tranche = "16 - 24"
					} else if (data[i].Vente < 32) {
						data[i].Tranche = "24 - 32"
					} else if (data[i].Vente < 40) {
						data[i].Tranche = "32 - 40"
					} else if (data[i].Vente < 48) {
						data[i].Tranche = "40 - 48"
					}else {
						data[i].Tranche = "> 48"
					}
		...
		pivot1 = new WebDataRocks({
			...
			"options": {
				"grid": {
					"type": "classic",
					"title": "VENTILATION FACTURES PAR TRANCHE QUANTITE",
					"showHeaders": false,
					"showTotals": "columns",
