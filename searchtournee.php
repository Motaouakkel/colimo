...
sel.appendChild(opt); 
  ...
  loadagency("agency.php");
								 ...
								 <div class="col-md-2" id="div-superviseur">
                                    <label class="field select">
                                        <select id="f-customer" name="f-customer" onchange="loadtournee(this.value)">
<script>
...
sel.appendChild(opt); 
      ...
      return;
									 ...
									 <div class="col-md-2" id="div-superviseur">
                                    <label class="field select">
                                        <select id="f-tournee" name="f-tournee" >
<script>
										  
										  function loadtournee(secteur) {

   /*make an HTTP request using the attribute value as the file name:*/
      xhttp = new XMLHttpRequest();
      xhttp.onreadystatechange = function() {
        if (this.readyState == 4) {
			var list = document.getElementById("f-tournee");
...
while (list.hasChildNodes()) {   
  list.removeChild(list.firstChild);
	...
	var sel12 = document.getElementById('f-tournee');
var opt12 = document.createElement('option');
opt12.appendChild( document.createTextNode("Tout") );
...
sel12.appendChild(opt12); 
		...
		var b = JSON.parse(this.response);
			...
			var dataSet = [];
			var sel = document.getElementById('f-tournee');
...
sel.appendChild(opt); 
      ...
      xhttp.open("POST", "tournee.php", true);
	  
    	xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	  xhttp.send("location_id="+secteur);
      ...
      return;
								...
								<div class="col-md-2">
                                      <a type="submit" href = "#" onclick="loadme(getsup(),getag())" class="button btn-primary submit-btn">Consulter</a>
