...
var ch =  1
      ...
      xhttp.open("POST", "acces5.php", true);
...
function loadfile(f) {
	...
	function getJSONData() {
			...
			for (var i= 1; i < data.length; i++) {
				if (i>1) {
					if (data[i].Secteur == data[i-1].Secteur) {
						data[i].Intra = (data[i].Sec_Scan - data[i-1].Sec_Time);
					}
					
					//if (data[i].Secteur not in distinct) {
					//	distinct.push(data[i].Secteur);
					//}
  
				}
				//for (var line in data) {
					//console.log(data[i].NumFacture)
					//data[i].Intra = 0;
				//}
			...
			unique = [...new Set(data.map(item => item.Secteur))];
