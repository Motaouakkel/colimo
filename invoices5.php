...
function loadfile(f) {
	...
	function getJSONData() {
			...
			for (var i= 1; i < data.length; i++) {
				...
				data[i].CA = (data[i].CA + data[i].Remise);
				
				if (data[i].CA < 75) {
					data[i].Tranche = "0 - 75"
				} else if (data[i].CA < 125) {
					data[i].Tranche = "075 - 125"
				} else if (data[i].CA < 175) {
					data[i].Tranche = "125 - 175"
				} else if (data[i].CA < 225) {
					data[i].Tranche = "175 - 225"
				} else if (data[i].CA < 275) {
					data[i].Tranche = "225 - 275"
				}else {
					data[i].Tranche = "> 275"
				}
		...
		pivot1 = new WebDataRocks({
			...
			report: {
				...
				"measures": [
					...
					{
						"uniqueName": "MoyCA",
						"formula": "sum(\"CA\")  / count(\"NumFacture\") ",
						"caption": "Moy.CA",
						"format": "3dyl7oh7"
					},
					{
			...
			"options": {
				"grid": {
					"type": "classic",
					"title": "VENTILATION FACTURES PAR TRANCHE C.A",
					"showHeaders": false,
					"showTotals": "columns",
	...
	webdatarocks.on('reportchange', function() {
		setTimeout(function() { calcul_cumul() }, 1500);
	});
