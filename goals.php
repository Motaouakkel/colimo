            ...
            <header id="topbar" class="wdr-ui-element">
                <div class="topbar-left">
                    <ol class="breadcrumb">
                        ...
                        <li class="crumb-trail">Objectifs</li>
                    </ol>
                </div>
...
function  loadme(sup_id,ag_id) {
	...
	var ch =  1
		...
		xhr.send("description="+ document.getElementById("date1").value + ' -> ' + document.getElementById("date1").value + ' ,filtres : ' + text1 + ',' + text2 + "&action_name="+'Indicateurs de performance/Période');
		  ...
		  xhttp.open("POST", "acces16.php", true);
...
function loadfile(f) {
	...
	function getJSONData() {
			...
			var struct = {
				...
				"sector": {type : "string"},
				"taxed_amount": {type : "number"},
				"goal": {type : "number"},
				"ca": {type : "number"},
				"NBJ": {type : "number"},
				"realisation": {type : "number"},
			...
			for (var i= 0; i < data.length; i++) {
					...
					data[i]['ca'] = (data[i]['taxed_amount'] / nb_jours);							
					data[i].NBJ = nb_jours;
					
					if (data[i]['goal'] > 0 ) {
						data[i]['realisation'] = (data[i]['taxed_amount'] / nb_jours) / data[i]['goal'] *100;	
					}
...
var pivot1 = new WebDataRocks({
        ...
        "measures": [
			...
			{
				"uniqueName": "realisation",
				"formula": "sum(\"realisation\")",
				"caption": "% Réalisation",
				"format": "3dhvwiax"
			}
        ]
